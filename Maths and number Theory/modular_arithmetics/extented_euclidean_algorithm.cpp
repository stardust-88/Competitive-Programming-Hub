...
ll mod_exp(ll x,ll n){ll res=1;while(n>0){if(n&1)res=(res*x)%mod;x=(x*x)%mod;n>>=1;}return res;}
void inout(){freopen("in.txt","r",stdin);freopen("out.txt","w",stdout);}

// time complexity: O(log(max(A,B)))

ll d,x,y;
void ext_euc(ll a,ll b){
  if(b==0){
    d=a;
    x=1;
    y=0;
...
int main(){
    ...
    cin>>a>>b;
    ext_euc(a,b);
    cout<<"gcd of a and b is:"<<" "<<d<<en;
    cout<<"coefficients x and y are:"<<" "<<x<<" "<<y<<en;
return 0;
}
